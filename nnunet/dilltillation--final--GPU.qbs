...
#PBS -j oe
#PBS -o dilltillation--final--GPU.log   # 合并输出和错误到一个文件
#PBS -q gentai
cd $PBS_O_WORKDIR
...
# 关键修改：使用特殊重定向方式实时输出日志
{
    ...
    exec > >(tee -a dilltillation--final--GPU.log) 2>&1
    
    # 输入需要执行的命令
    nnUNetv2_predict -d 15 -i /home/fanggang_1/hk/nnunet/nnUNet/spin -o /home/fanggang_1/hk/nnunet/nnUNet/SPoutnnUNetTrainerLightweightDistillation2nd -f 0 -tr nnUNetTrainerLightweightDistillation2nd -c 3d_lowres -p nnUNetPlans

    # 显示结束时间和退出状态
    echo "Exit status: $?"
    echo "End time: $(date)"
    echo "==================== JOB FINISHED ===================="
} | tee -a dilltillation--final--GPU.log
