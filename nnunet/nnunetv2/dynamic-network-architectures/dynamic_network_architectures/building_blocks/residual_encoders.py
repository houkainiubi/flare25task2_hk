...
from dynamic_network_architectures.building_blocks.lightweight_residual import LightweightBasicBlock, MobileInvertedResidualBlock
from dynamic_network_architectures.building_blocks.lightweight_pooling import get_lightweight_pool_op, get_efficient_downsampling
...
class ResidualEncoder(nn.Module):
    def __init__(self,
                 ...
                 lightweight_block_type: str = 'basic',  # 'basic', 'mobile_inverted'
                 lightweight_pool_type: str = 'adaptive_avg',  # 'adaptive_avg', 'stride_conv', 'attention'
                 mobile_expand_ratio: int = 6
                 ):
        """
        ...
        :param lightweight: if True, use lightweight residual blocks and pooling operations
        :param lightweight_block_type: 'basic' for lightweight basic block, 'mobile_inverted' for MobileNetV2-style blocks
        :param lightweight_pool_type: pooling type for lightweight mode ('adaptive_avg', 'stride_conv', 'attention')
        :param mobile_expand_ratio: expansion ratio for mobile inverted residual blocks
        """
        super().__init__()
        ...
        # 根据轻量化设置选择池化操作
        if lightweight and lightweight_pool_type != 'conv':
            pool_op = get_lightweight_pool_op(conv_op, lightweight_pool_type) if pool_type != 'conv' else None
        else:
        ...
        for s in range(n_stages):
            ...
            if lightweight:
                # 使用轻量化残差块
                if lightweight_block_type == 'mobile_inverted':
                    # 使用MobileNetV2风格的倒残差块
                    stage_blocks = []
                    for b in range(n_blocks_per_stage[s]):
                        block_stride = stride_for_conv if b == 0 else 1
                        stage_blocks.append(
                            MobileInvertedResidualBlock(
                                conv_op, input_channels if b == 0 else features_per_stage[s], 
                                features_per_stage[s], kernel_sizes[s], block_stride,
                                expand_ratio=mobile_expand_ratio, conv_bias=conv_bias,
