...
class nnUNetPredictor(object):
            ...
            if of is not None:
                ...
                if self.optimize_postprocessing:
                    ...
                    if idx == 0:  # 只在第一个案例时打印系统信息
                        ...
                        print(f"逻辑CPU核心数: {logical_cores}")
                        print(f"系统内存: {memory_gb:.1f} GB")
                        print(f"重采样线程数: {min(physical_cores, 12)}")
                        print(f"后处理线程数: {min(physical_cores, 12)}")
                        print(f"文件I/O线程数: 2")
                        print("="*70)
                        print("✨ 优化策略:")
                        print("  • 并行重采样 - 按类别分割处理")
    ...
    def _parallel_resample_prediction(self, prediction_logits: np.ndarray, 
        ...
        优化版本：改进负载均衡和错误处理，使用12线程
        """
        if num_workers is None:
            num_workers = min(psutil.cpu_count(logical=False), 12)  # 改为12线程
        ...
        print(f"重采样缩放因子: {zoom_factors}")
        
        num_classes = prediction_logits.shape[0]
        
        # 优化：预估计算复杂度并调整线程数
        total_elements = np.prod(target_shape) * num_classes
        memory_gb = total_elements * 4 / (1024**3)  # float32
        
        # 根据内存大小和计算复杂度动态调整线程数
        if memory_gb > 8.0:  # 大于8GB时减少线程数避免内存竞争
            num_workers = min(num_workers, 8)
            print(f"⚠️  大内存数据({memory_gb:.1f}GB)，降低并行度到 {num_workers} 线程")
        elif memory_gb > 4.0:  # 4-8GB时适中调整
            num_workers = min(num_workers, 12)
            print(f"⚠️  中等内存数据({memory_gb:.1f}GB)，调整并行度到 {num_workers} 线程")
        
        # 为每个类别并行执行重采样，增加错误处理和超时机制
        def resample_single_class(class_idx):
            ...
            try:
                ...
                if elapsed > 10.0:  # 超过10秒警告
                    print(f"⚠️  类别 {class_idx} 重采样耗时 {elapsed:.2f}s (形状: {class_prediction.shape} -> {resampled.shape})")
                
                return class_idx, resampled
                
            except Exception as e:
                print(f"❌ 类别 {class_idx} 重采样失败: {e}")
                # 返回零填充的结果作为fallback
                fallback_result = np.zeros(target_shape, dtype=np.float32)
                return class_idx, fallback_result
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            # 收集结果，增加超时处理
            processed_count = 0
            for future in as_completed(future_to_class, timeout=300):  # 5分钟超时
                try:
                    class_idx, resampled_data = future.result(timeout=60)  # 单个任务1分钟超时
                    resampled_predictions[class_idx] = resampled_data
                    processed_count += 1
                    
                    if processed_count % max(1, num_classes // 4) == 0:  # 进度报告
                        print(f"重采样进度: {processed_count}/{num_classes} ({100*processed_count/num_classes:.1f}%)")
                        
                except Exception as e:
                    class_idx = future_to_class[future]
                    print(f"❌ 类别 {class_idx} 处理异常: {e}")
                    # 使用零填充作为fallback
                    resampled_predictions[class_idx] = np.zeros(target_shape, dtype=prediction_logits.dtype)
    ...
    def _parallel_segmentation_postprocess(self, resampled_logits: np.ndarray, 
        ...
        # 分析数据形状以优化分块策略
        data_shape = resampled_logits.shape[1:]  # [z, y, x]
        total_elements = np.prod(data_shape)
        memory_gb = total_elements * resampled_logits.shape[0] * 4 / (1024**3)
        
        print(f"后处理数据形状: {resampled_logits.shape}, 内存占用: {memory_gb:.2f}GB")
        
        # 根据数据大小和线程数动态选择分块策略
        if memory_gb > 4.0:  # 大数据集使用更多分块，充分利用12线程
            chunk_size = max(1, data_shape[0] // (num_workers * 3))  # 创建更多小块以充分利用12线程
        elif memory_gb > 2.0:  # 中等偏大数据集
            chunk_size = max(1, data_shape[0] // (num_workers * 2))
        elif memory_gb > 1.0:  # 中等数据集
            chunk_size = max(1, data_shape[0] // (num_workers * 1.5))
        else:  # 小数据集
            chunk_size = max(1, data_shape[0] // num_workers)
        
        # 确保有足够的块来利用所有线程
        min_chunks_needed = num_workers * 2  # 至少是线程数的2倍
        if data_shape[0] // chunk_size < min_chunks_needed:
            chunk_size = max(1, data_shape[0] // min_chunks_needed)
        
        print(f"分块策略: 块大小={chunk_size}, 预计块数={data_shape[0] // chunk_size + (1 if data_shape[0] % chunk_size else 0)}")
        
        # 分块处理大体积数据，增加错误处理
        def process_chunk(chunk_data, start_slice, end_slice, chunk_id):
            """处理数据块，增加错误处理和性能监控"""
            try:
                start_time = time.time()
                
                # 应用softmax（使用更高效的numpy实现）
                # 为避免数值溢出，先减去最大值
                chunk_data_stable = chunk_data - np.max(chunk_data, axis=0, keepdims=True)
                chunk_exp = np.exp(chunk_data_stable)
                chunk_softmax = chunk_exp / np.sum(chunk_exp, axis=0, keepdims=True)
                
                # 获取最终分割（argmax）- 使用uint8节省内存
                chunk_segmentation = np.argmax(chunk_softmax, axis=0).astype(np.uint8)
                
                # 应用标签映射（如果需要）
                if hasattr(label_manager, 'all_labels') and len(label_manager.all_labels) > 0:
                    # 创建映射表以提高效率
                    label_map = np.arange(256, dtype=np.uint8)  # 默认映射
                    for i, label in enumerate(label_manager.all_labels):
                        if 0 <= i < 256 and 0 <= label < 256:
                            label_map[i] = label
                    
                    # 应用映射
                    chunk_segmentation = label_map[chunk_segmentation]
                
                elapsed = time.time() - start_time
                if elapsed > 5.0:  # 超过5秒警告
                    print(f"⚠️  块 {chunk_id} 后处理耗时 {elapsed:.2f}s (切片 {start_slice}-{end_slice})")
                
                return start_slice, end_slice, chunk_segmentation, True
                
            except Exception as e:
                print(f"❌ 块 {chunk_id} (切片 {start_slice}-{end_slice}) 处理失败: {e}")
                # 返回零填充结果作为fallback
                fallback_shape = chunk_data.shape[1:]  # 去掉类别维度
                fallback_result = np.zeros(fallback_shape, dtype=np.uint8)
                return start_slice, end_slice, fallback_result, False
        
        # 按Z轴分块并行处理，确保负载均衡
        z_dim = resampled_logits.shape[1]
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            for start_z in range(0, z_dim, chunk_size):
                ...
                future = executor.submit(process_chunk, chunk, start_z, end_z, chunk_id)
                futures.append(future)
                chunk_id += 1
            
            print(f"创建了 {len(futures)} 个处理块，每块约 {chunk_size} 切片")
            
            # 组装结果，增加进度监控
            processed_chunks = 0
            successful_chunks = 0
            
            for future in as_completed(futures, timeout=180):  # 3分钟总超时
                try:
                    start_slice, end_slice, chunk_result, success = future.result(timeout=30)  # 单块30秒超时
                    final_segmentation[start_slice:end_slice] = chunk_result
                    processed_chunks += 1
                    if success:
                        successful_chunks += 1
                    
                    if processed_chunks % max(1, len(futures) // 4) == 0:  # 进度报告
                        print(f"后处理进度: {processed_chunks}/{len(futures)} 块 ({100*processed_chunks/len(futures):.1f}%)")
                        
                except Exception as e:
                    print(f"❌ 后处理块异常: {e}")
                    processed_chunks += 1
            
            if successful_chunks < len(futures):
                print(f"⚠️  {len(futures) - successful_chunks} 个块处理失败，使用零填充")
    ...
    def _optimized_export_prediction_from_logits(self, predicted_array_or_file, 
        ...
        input_shape = prediction_logits.shape
        memory_estimate = np.prod(original_shape) * prediction_logits.shape[0] * 4 / (1024**3)
        
        print(f"输入形状: {input_shape}")
        print(f"目标形状: {original_shape}")
        print(f"预估内存需求: {memory_estimate:.2f}GB")
        
        # 根据内存需求调整策略
        if memory_estimate > 8.0:
            print("⚠️  大内存数据集，启用保守处理策略")
            # 强制垃圾回收
            gc.collect()
        ...
        try:
            ...
            print(f"重采样后内存: {current_memory:.2f}GB (增加: {current_memory-initial_memory:.2f}GB)")
            ...
            if save_probabilities:
                ...
                try:
                    # 使用更内存友好的方式计算softmax
                    # 分块处理避免内存峰值
                    chunk_size = min(64, resampled_logits.shape[1])  # 每次处理64个切片
                    probabilities = np.zeros_like(resampled_logits, dtype=np.float16)  # 使用float16节省内存
                    
                    for start_idx in range(0, resampled_logits.shape[1], chunk_size):
                        end_idx = min(start_idx + chunk_size, resampled_logits.shape[1])
                        chunk = resampled_logits[:, start_idx:end_idx]
                        
                        # 计算softmax并转换为float16
                        chunk_softmax = torch.softmax(torch.from_numpy(chunk), dim=0).numpy().astype(np.float16)
                        probabilities[:, start_idx:end_idx] = chunk_softmax
            ...
            # 监控内存使用
            current_memory = process.memory_info().rss / (1024**3)
            print(f"处理完成后内存: {current_memory:.2f}GB")
            ...
            # 总结性能
            total_time = resample_time + postprocess_time + (prob_time if save_probabilities else 0) + save_time
            print(f"🎯 深度优化总耗时: {total_time:.2f}s")
            print("✅ 深度多线程后处理优化完成")
            
        except Exception as e:
            print(f"❌ 后处理优化过程中发生错误: {e}")
            # 尝试fallback到标准处理方式
            print("🔄 回退到标准处理方式...")
            from nnunetv2.inference.export_prediction import export_prediction_from_logits
            export_prediction_from_logits(
                predicted_array_or_file, properties_dict, configuration_manager, 
                plans_manager, dataset_json_dict_or_file, output_file_truncated, save_probabilities
            )
